...
const productsIndexedByCategory = data.products.reduce(
	(accumulatorObj, item) => {
...
function setLocalCounter() {
	if (!localStorage.hasOwnProperty("orderCounter")) {
		...
		localStorage.setItem(
			"orderCounter",
			localStorage.getItem("orderCounter")
		);
	}
}
...
document
	.querySelector(`[data-return-button="true"]`)
	.addEventListener("click", setLogicReturnBtn);
...
function renderDetails() {
	const product = productsIndexedByCategory[activeCategory].find(
		item => item.title === activeProduct
	);
...
function handlePurchase(event) {
	if (event.target.hasAttribute("data-purchase")) {
		...
		document.querySelector(".order-section__title").textContent =
			targetProduct;
		document
			.querySelector(".order-section")
			.classList.add("order-section_show");
	...
	if (isValid) {
		...
		localStorage.setItem(
			...
			JSON.stringify({
				...
				productPrice: `${
					productsIndexedByCategory[targetCategory].find(
						item => item.title === targetProduct
					).price
				}$`,
		...
		document
			.querySelector(".order-section")
			.classList.remove("order-section_show");
		renderCategories();
	}
};
...
function renderOrders() {
	...
	document
		.querySelector(`[data-return-button="true"]`)
		.classList.remove("cart-section__button_theme_hidden");
...
function handleOrders(event) {
	...
	if (event.target.hasAttribute("data-detailorder")) {
		const objTemp = JSON.parse(
			localStorage[event.target.dataset.detailorder]
		);

		const detailMessage = document.createElement("div");
		detailMessage.classList.add("orders-show-section__help");
		detailMessage.setAttribute(
			"data-helporder",
			event.target.dataset.detailorder
		);
		...
		if (
			document.querySelector(
				`[data-helporder="${+event.target.dataset.detailorder}"]`
			)
		) {
			document
				.querySelector(
			...
			document
				.querySelector(
					`[data-detailorder="${+event.target.dataset.detailorder}"]`
				)
				.after(detailMessage);
		}
	}
