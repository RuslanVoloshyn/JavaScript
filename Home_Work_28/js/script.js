...
form.onsubmit = event => {
	...
	if (isValid) {
		alert(
			`${objNameValue.pib} замовив ${targetProduct} у кількості ${objNameValue.amount}шт. у місто ${objNameValue.city} на склад НП ${objNameValue.storage}. Обраний метод оплати: ${objNameValue.typeOfPay}`
		);
		document
			.querySelector(".order-section")
